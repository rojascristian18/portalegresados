 ...
 * - Create a subclass of AppExceptionRenderer and configure it to be the `Exception.renderer`
 *
 ...
 *
 * #### Using a subclass of AppExceptionRenderer
 *
 * Using a subclass of AppExceptionRenderer gives you full control over how Exceptions are rendered, you
 ...
 */
	public function __construct(Exception $exception) {
		 ...
		 * Respuestas API
		 */
		if ( $this->controller->Components->enabled('RequestHandler') && $this->controller->RequestHandler->ext === 'json' )
		{
			$method		= 'errorApi';
		}
 ...
 */
	public function errorApi($error)
	{
		...
		$message		= ($error->getMessage() ? $error->getMessage() : $attributes['message']);
		$code			= (! empty($attributes['code']) ? $attributes['code'] : 500);
		if ( ! Configure::read('debug') || ! $message )
		{
		...
		{
			$set['details'] = $attributes['details'];
			array_push($set['_serialize'], 'details');
		}
